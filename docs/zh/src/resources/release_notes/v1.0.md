...
下文的版本概览与主要能力描述了 `v1.0.0` 标签中可用的 API 与功能。
后续更新见 [v1.0 补丁版本](#v10-补丁版本)。
...
### v1.0.1 — 2026 年 9 月 22 日

[v1.0.1](https://github.com/antgroup/vsag/releases/tag/v1.0.1) 是 1.0 LTS 系列的首个补丁版本，包含 17 项变更，主要涉及索引正确性、DiskANN 内存使用和构建稳定性。

- **HGraph：** 移除 `ef_search` 与 `topk` 关联的上限限制（[#2525](https://github.com/antgroup/vsag/pull/2525)），允许遍历距离为非有限值的桥接节点（[#2806](https://github.com/antgroup/vsag/pull/2806)），修复强制删除尾部节点后入口点失效的问题（[#2906](https://github.com/antgroup/vsag/pull/2906)）。
- **其他索引修复：** 回收 BruteForce 强制删除后的存储空间（[#2538](https://github.com/antgroup/vsag/pull/2538)），跳过 SINDI 范围检索中距离为零的候选（[#2715](https://github.com/antgroup/vsag/pull/2715)），对齐 Pyramid 原始向量读取行为（[#2760](https://github.com/antgroup/vsag/pull/2760)）。
- **DiskANN：** 避免为检查大小而复制流内容（[#2630](https://github.com/antgroup/vsag/pull/2630)），限制最近中心计算的分块内存使用（[#2646](https://github.com/antgroup/vsag/pull/2646)）。
- **依赖配置：** 新增带固定版本限定的第三方依赖覆盖配置（[#2686](https://github.com/antgroup/vsag/pull/2686)）。
- **OpenBLAS 与打包：** 升级内置 OpenBLAS 并稳定 x86 平台检测（[#2704](https://github.com/antgroup/vsag/pull/2704)、[#2876](https://github.com/antgroup/vsag/pull/2876)），进一步修复 manylinux 依赖构建（[#2971](https://github.com/antgroup/vsag/pull/2971)）。
- **编译器兼容性：** 修复 GCC 15 下的头文件编译问题（[#2976](https://github.com/antgroup/vsag/pull/2976)），避免 GCC 14 下稀疏向量去重复制产生的警告（[#2992](https://github.com/antgroup/vsag/pull/2992)）。
- **CI 与测试：** 恢复 1.0 分支的 PR lint 检查（[#2488](https://github.com/antgroup/vsag/pull/2488)），将 OpenBLAS 临时处理限定在常规 TSan 任务（[#2670](https://github.com/antgroup/vsag/pull/2670)），并在测试中容忍 INT8 余弦计算的舍入误差（[#2968](https://github.com/antgroup/vsag/pull/2968)）。

完整变更参见 [v1.0.0...v1.0.1 对比](https://github.com/antgroup/vsag/compare/v1.0.0...v1.0.1)；完整 PR 清单和贡献者名单参见[官方 Release](https://github.com/antgroup/vsag/releases/tag/v1.0.1)。
